...
class Builder extends AbstractModel
{
    ...
    {
        if (isset($data['shipping']['country_code']) && 'PR' === strtoupper($data['shipping']['country_code'])) {
            $data['shipping']['country_code'] = 'US';
            $data['shipping']['state'] = 'PR';
        }
        ...
        if (isset($data['shipping_address']['state'])) {
            ...
            $isExclude = in_array(strtoupper($data['shipping_address']['state']), $excludeTaxStates, true);
