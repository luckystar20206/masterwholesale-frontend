    ...
    $.widget('bss.Sdcp', $.mage.SwatchRenderer, {
        ...
        _UpdateActiveTab: function () {
            ...
            if ($(window.location).attr('hash') == '') {
                $('.data.item.title:not(.' +this.options.sdcp_classes.hiddenTab+ ')').first().trigger('click');
            }
        },
        ...
        _UpdateStock: function ($status, $number, $config, $custom_stock_status, $custom_shipping, $backorders) {
            if ($config > 0) {
                /*var stock_status = 'Availablity: ';
                if ($status > 0) {
                    stock_status += Number($number) + ' ' + $t('in store');
                    $(this.options.sdcp_classes.addtocart_button).removeAttr('disabled');
                } else {
                    stock_status = $t('0 in store');
                    $(this.options.sdcp_classes.addtocart_button).attr('disabled', 'disabled');
                }
                
                $(this.options.sdcp_classes.stock).html(stock_status);*/
                //var stock_status = '<strong>Availablity:</strong> ';
                var custom_ship = '<p><strong>Shipping:</strong> '+$custom_shipping+'</p>';
                var custom_stock = '';
                if ($number > 0) {
                    custom_stock += '<p><strong>Availability:</strong> '+$custom_stock_status+'</p>';
                    $('.box-tocart .actions .tocart').removeAttr('disabled');
                } else {
                    if($backorders){
                        custom_stock += '<p><strong>Availability:</strong> '+$custom_stock_status+'</p>';
                        // custom_stock += '<p><strong>Availability:</strong> Backorders</p>';
                    }else{
                        custom_stock += '<p><strong>Availability:</strong> Out of stock</p>';
                        $('.box-tocart .actions .tocart').attr('disabled', 'disabled');
                    }
                }
                $('.custom_stock_shipping').html(custom_stock+custom_ship);
        ...
        _ResetStock: function ($config) {
            if ($config > 0) {
                ...
                if (this.options.jsonChildProduct['stock_status'] > 0) {
                    stock_status = $t('Mixed Availability');
                    $(this.options.sdcp_classes.addtocart_button).removeAttr('disabled');
                } else {
                    stock_status = $t('Currently Out of Stock');
                    $(this.options.sdcp_classes.addtocart_button).attr('disabled', 'disabled');
                }
                $(this.options.sdcp_classes.stock).html(stock_status);
            }
