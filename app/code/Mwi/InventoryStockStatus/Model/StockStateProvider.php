...
namespace Mwi\InventoryStockStatus\Model;
        ...
        if (!$this->checkQty($stockItem, $summaryQty) || !$this->checkQty($stockItem, $qty)) {
            ...
            if ($stockItem->getQty() - $summaryQty < 0) {
                if ($stockItem->getProductName()) {
                    ...
                    $date_available = strtotime($product->load($stockItem->getProductId())->getData('date_available'));


                    //print($stockItem->getProductId() .' ' .$date_available ."<br>");



                    if($date_available > time()){
                        $bo_msg = 'Item on backorder until ' .date('m/d',strtotime($product->getData('date_available'))) .'. Item will ship when it is back in stock.';
                    } elseif($product->getData('stock_status') == 37357){//in stock
                        $bo_msg = 'The quantity requested (' .$backOrderQty .') is on order and will ship when it arrives.';
                    } elseif($product->getData('stock_status') == 37359 && $product->getData('manufacturer') == 37089){//Schluter ships from store or factory
                        $bo_msg = 'Please allow up to 12 days for delivery when choosing USPS Priority Mail shipping for this item.';
                    } else {
                        $bo_msg = 'The quantity ordered (' .$backOrderQty .') will ship direct from manufacturer.';
                        $itemDropShip = true;
                    }
