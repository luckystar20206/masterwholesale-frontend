...
class Builder extends AbstractModel
{
    ...
    {
        if (isset($data['shipping']['address']['state']) && 'PR' === strtoupper($data['shipping']['address']['state'])) {
            $data['shipping']['address']['country_code'] = 'US';
            $data['shipping']['address']['state'] = 'PR';
        }

        if (isset($data['shipping']['address']['state'])) {
            $excludeTaxStates = array(
                'AL', 'AZ', 'AR',
                'CA', 'CO', 'CT',
                'DC', 'HI', 'ID',
                'IL', 'IN', 'IA',
            ...
            $isExclude = in_array(strtoupper($data['shipping']['address']['state']), $excludeTaxStates, true);
            $isExclude && $data['tax_details']['skip_tax'] = true;
        }
