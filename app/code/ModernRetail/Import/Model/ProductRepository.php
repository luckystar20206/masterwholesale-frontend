 ...
 */
class ProductRepository extends \Magento\Catalog\Model\ProductRepository implements \Magento\Catalog\Api\ProductRepositoryInterface
{
    /**
    ...
    public function __construct(
        ...
        if(class_exists( '\Magento\Framework\Api\SearchCriteria\CollectionProcessorInterface')) {
            $collectionProcessor = $om->create('\Magento\Framework\Api\SearchCriteria\CollectionProcessorInterface');
        }else {
            $collectionProcessor = null;
        }
        if(class_exists( '\Magento\Framework\Serialize\Serializer\Json')) {
            $serializer = $om->create('\Magento\Framework\Serialize\Serializer\Json');
        }else{
            $serializer = null;
        }
        ...
        parent::__construct($productFactory, $initializationHelper, $searchResultsFactory, $collectionFactory, $searchCriteriaBuilder, $attributeRepository, $resourceModel,
            $linkInitializer, $linkTypeProvider, $storeManager, $filterBuilder, $metadataServiceInterface, $extensibleDataObjectConverter, $optionConverter, $fileSystem, $contentValidator,
            $contentFactory, $mimeTypeExtensionMap,  $imageProcessor, $extensionAttributesJoinProcessor,$collectionProcessor, $serializer,$cacheLimit
        );
    }
     ...
     * @param string $cacheKey
     */
    private function saveProductInLocalCache($product, string $cacheKey)
    {
    ...
    private function saveProduct($product)
    {
        try {
            ...
            throw new CouldNotSaveException(__('Unable to save product'. $e->getMessage()), $e);
        }
    }
}
